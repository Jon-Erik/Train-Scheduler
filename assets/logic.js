...
//pushes inputed train information to database
...
//Whenever a new train is added to the database, a new row is displayed with its information in the schedule.
database.ref().orderByChild("start date").on("child_added", function(snapshot) {
	...
	newRow.append(frequencyTD);
		...
		//Calculates the time of the next arrival and minutes away using moment.js based on the first deparature time, the frequency, and the present time
...
//These two blocks of code allow deletions from the database and the HTML based on the unique ID generated by Firebase.
